...
$query="INSERT INTO codigo_alumno (id_cod_al,id_cod,id_alumno,id_profesor,fecha) VALUES (0,'$codigo','$alumno','$profesor','$fecha')";
	...
	function  consultarCodigoAlumno($alumno){
		...
		$sql = $this->db->query("SELECT codigo_alumno.fecha,codigo_alumno.id_cod_al,codigo.nombre AS codigoo,
 								 ...
 								 FROM (((escuela_lice.codigo_alumno codigo_alumno
 								   INNER JOIN escuela_lice.profesores profesores
             						ON (codigo_alumno.id_profesor = profesores.id_profesor))
         							INNER JOIN escuela_lice.alumno alumno ON (codigo_alumno.id_alumno = alumno.id_alumno))
        							INNER JOIN escuela_lice.codigo codigo ON (codigo_alumno.id_cod = codigo.id_cod))
     								INNER JOIN escuela_lice.tipo_codigo tipo_codigo ON (codigo.id_tip_cod = tipo_codigo.id_tip_cod)
