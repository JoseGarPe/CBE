...
function cargarSeccion($seccion,$profesor){
		...
		$sql = $this->db->query('SELECT secc.id_seccion as id, secc.nombre as nombre FROM detalle_materia dtm INNER JOIN detalle_horario dth ON dtm.id_detalle_horario=dth.id_detalle_horario INNER JOIN asignacion_materia asm ON dth.id_asignacion_materia=asm.id_asignacion_materia INNER JOIN profesores prf ON prf.id_profesor=asm.id_profesor INNER JOIN detalle_grado dtg ON dtm.id_detalle_grado=dtg.id_detalle_grado INNER JOIN grado grd ON dtg.id_grado=grd.id_grado INNER JOIN seccion secc ON dtg.id_seccion=secc.id_seccion WHERE prf.id_profesor = "'.$profesor.'" AND dtg.id_grado="'.$seccion.'"'); 
