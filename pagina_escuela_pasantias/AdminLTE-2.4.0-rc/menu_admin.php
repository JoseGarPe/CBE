      <ul class="sidebar-menu" data-widget="tree">
        <li class="header">MENU</li>
            <li class="active treeview">
              <a href="#">
            <i class="fa fa-edit"></i> <span>Asignaciones</span>
            <span class="pull-right-container"> <i class="fa fa-angle-left pull-right"></i> </span>
              </a>
             <ul class="treeview-menu">
                  <li><a href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_detalle_actividad.php"><i class="fa fa-circle-o"></i>Asig. Actividades</a></li>
                  <li><a href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_asignacion_materia.php"><i class="fa fa-circle-o"></i>Asig. Materias</a></li>
                  <li><a href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/asignacion_horario.php"><i class="fa fa-circle-o"></i>Asig. Horarios</a></li>
                  <li><a href="../listas/notas_administrador.php"><i class="fa fa-circle-o"></i>Asig. Notas</a></li>
                  <li><a href="../listas/asistencia_administrador.php"><i class="fa fa-circle-o"></i>Asistencias</a></li>
              </ul>
            </li>



        <li class="treeview">
          <a href="#">
            <i class="fa fa-book"></i> <span>Administracion</span>
            <span class="pull-right-container">
              <i class="fa fa-angle-left pull-right"></i>
            </span>
          </a>
          <ul class="treeview-menu">
              <li><a class="" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_actividadPeriodo.php"><i class="fa fa-circle-o"></i>Actividades</a></li>
               <li><a class="" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_aviso.php"><i class="fa fa-circle-o"></i>Avisos</a></li>
              <li><a class="" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_periodo.php"><i class="fa fa-circle-o"></i>Periodos</a></li>
              <li><a class="" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_admins.php"><i class="fa fa-circle-o"></i>Administrador</a></li>
              <li><a class="" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_profesores.php"><i class="fa fa-circle-o"></i>Profesores</a></li>
              <li><a class="" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_inscripcion.php"><i class="fa fa-circle-o"></i>Inscripciones</a></li>
              <li><a class="" href="../listas/lista_alumno.php"><i class="fa fa-circle-o"></i>Alumnos</a></li>
            <li class="treeview">
                  <a href="#">
                    <i class="fa fa-book"></i> <span>Asesores legales</span>
                   <span class="pull-right-container">
                    <i class="fa fa-angle-left pull-right"></i>
                   </span>
                 </a>
                <ul class="treeview-menu">
                    <li> <a class="dropdown-item" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_responsables.php">Responsables</a></li>
                    <li> <a class="dropdown-item" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_madres.php">Madres</a></li>
                   <li>  <a class="dropdown-item" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_padres.php">Padres</a></li>
                </ul>
               </li>
              <li><a class="" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_ma.php"><i class="fa fa-circle-o"></i>Mensualidades</a></li>
              <li><a class="" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_seccion.php"><i class="fa fa-circle-o"></i>Secciones</a></li>
              <li><a class="" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/observacion_administrador.php"><i class="fa fa-circle-o"></i>Observaciones</a></li>
              <li><a class="" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_materia.php"><i class="fa fa-circle-o"></i>Materia</a></li>
              <li><a class="" href="#"><i class="fa fa-circle-o"></i>Horario</a></li>
              <li><a class="" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_area.php"><i class="fa fa-circle-o"></i>Areas</a></li>  
              <li><a class="" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/listas/lista_codigo.php"><i class="fa fa-circle-o"></i>Codigos</a></li>  
              <li><a class="" href="/CBE-master/pagina_escuela_pasantias/adminLTE-2.4.0-rc/registros/contra_admins.php"><i class="glyphicon glyphicon-random"></i>    Cambiar Contraseña</a></li>      
           </ul>
        </li>

       
      </ul>